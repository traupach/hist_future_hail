...
\begin{figure}[!ht]
    ...
    \caption{Densities maximum daily hail size per domain and epoch derived from WRF simulations and the fitted GEV models.}
    \label{fig:densities_hail}
\end{figure}

\begin{figure}[!ht]
    \includegraphics[width=\textwidth]{figures/gev_dists_wind}
    \caption{As for Figure \ref{fig:densities_hail}, but for daily maximum 10 m wind collocated with hail.}
...
\begin{figure}[!ht]
    ...
    \caption{Distributions of $p$ values from KS tests, comparing empirical
     values GEV models, and historical GEVs to future GEVs, per variable and
     domain. For each test, the KS test was applied 100 times with 1000 random
     values drawn from the relevant GEV distribution(s) each time, to obtain a
...
\begin{table}
    \centering
    \caption{Probability of a hail day exceeding hail sizes of 20, 50, or 100 mm, and of coincident wind exceeding 80 and 100 km h$^{-1}$, per domain and epoch.}
...
\begin{table} 
    ...
    \todo{check in code}. Variables are convective available potential energy
    (CAPE, in J kg$^{-1}$), convective inhibition (CIN, in J kg$^{-1}$),
    freezing level height (FLH, in m), hail size (in mm), lifted index (LI, in
    K), lapse rate between 700 hPa and 500 hPa (LR, in K km$^{-1}$), melting
    level height (MLH, in m), bulk vertical wind shear between 0 and 6 km (S06,
    in m s$^{-1}$), temperature at 500 hPa (T500, in K), and 10 m wind (Wind, in
    m s$^{-1}$).}
...
\begin{table}
    ...
    \cite<e.g.>{Warren_QJRMS_2020}, and that MESH data are subject to
